...
# While paused it probes after the recorded reset (and every 20 min before it, since windows can reset
# early) and lifts the pause once both windows read under the thresholds.
...
under_bar() { python3 -c "import sys;sys.exit(0 if float('$1')<$PRIMARY_MAX and float('$2')<$WEEKLY_MAX else 1)"; }

LAST_PROBE=0
while :; do
  read -r P W PR WR < <(read_limits)
  ...
  if [ -f "$RUN/QUOTA_PAUSE" ]; then
    ...
    NOW=$(date +%s)
    # probe after the recorded reset, and every 20 min before it: windows sometimes reset early
    if [ -n "$RESET_AT" ] && { [ "$NOW" -ge $((RESET_AT + 120)) ] || [ $((NOW - LAST_PROBE)) -ge 1200 ]; }; then
      LAST_PROBE=$NOW
      timeout 180 codex exec --skip-git-repo-check -m "$TESTER_MODEL" -c model_reasoning_effort=low "reply ok" >/dev/null 2>&1
      read -r P W PR WR < <(read_limits)   # the probe wrote fresh rate_limits
      if under_bar "$P" "$W"; then
